...
namespace XWorld
{
    public class ResourcesProxy
    {
        ...
        private static Int32 m_LastGCTime = -1;
        private static Int32 m_GCTotalMemory = -1;
        public const string ROOT_PATH = "Assets/AssetDatas/";

#if UNITY_EDITOR
        public static UnityEngine.Object EditorLoadAsset(string relativePath)
        {
            string fullPath = ROOT_PATH + relativePath;
            return UnityEditor.AssetDatabase.LoadMainAssetAtPath(fullPath);
        }

        public static UnityEngine.Object[] EditorLoadAssets(string[] relativePaths)
        {
            int length = relativePaths.Length;
            UnityEngine.Object[] targets = new UnityEngine.Object[length];

            for (int i = 0; i < length; i++)
            {
                string fullPath = ROOT_PATH + relativePaths[i];
                targets[i] = UnityEditor.AssetDatabase.LoadMainAssetAtPath(fullPath);
            }
            return targets;
        }
#endif 
