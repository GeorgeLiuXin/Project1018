// ------------------------------------------------------------------------------
//  <autogenerated>
//      This code was generated by a tool.
//      Mono Runtime Version: 2.0.50727.1433
// 
//      Changes to this file may cause incorrect behavior and will be lost if 
//      the code is regenerated.
//  </autogenerated>
// ------------------------------------------------------------------------------

namespace XWorld.DataConfig
{
    using UnityEngine;
    using System.Collections.Generic;
    using System;
    
    
    public class client_config_define : System.ICloneable
    {
        
        public string tableKey;
        
        public string tableName;
        
        public string tableClassName;
        
        public string tablePath;
        
        public int tableInitOrder;
        
        public object Clone()
        {
            return this.MemberwiseClone();;
        }
    }
    
    public class Tableclient_config_define : XWorld.DataConfig.TableBase
    {
        
        public List<client_config_define> m_configList = new List<client_config_define>();
        
        public void LoadData(XWorld.DataConfig.client_config_define codeValue)
        {
            m_configList.Add(codeValue);
        }
        
        public int GetRowCount()
        {
            return 5;
        }
        
        public int GetDataCount()
        {
            if (m_configList == null)
            {
                return 0;
            }
            return m_configList.Count;
        }
        
        public client_config_define GetData(int rowIdx)
        {
            if (m_configList != null && rowIdx >= 0 && rowIdx < m_configList.Count)
            {
                return m_configList[rowIdx];
            }
            return null;
        }
        
        public override void LoadData(string content)
        {
            string[] values = content.Split("\r"[0]);
            for (int i = 2; (i < values.Length); i = (i + 1))
            {
                client_config_define data = new client_config_define();
                int j = 0;
                string[] subValues = values[i].TrimStart('\n').Split(ClientConfigManager.CMD_STRING, StringSplitOptions.None);
                if (subValues != null && subValues.Length == GetRowCount())
                {
                    data.tableKey = (subValues[j]);
                    j = j + 1;
                    data.tableName = (subValues[j]);
                    j = j + 1;
                    data.tableClassName = (subValues[j]);
                    j = j + 1;
                    data.tablePath = (subValues[j]);
                    j = j + 1;
                    data.tableInitOrder = ClientConfigManager.ToInt32(subValues[j]);
                    j = j + 1;
                    m_configList.Add(data);
                }
            }
        }
    }
}
