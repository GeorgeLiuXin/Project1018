...
namespace Galaxy
{
    ...
    public class GalaxySceneManager : GalaxyGameManagerBase
    {
        ...
        public override void InitManager()
        {
            ...
            GalaxyGameModule.GetGameManager<GalaxyPacketProcess>().AddPacketHandler("GPacketDynamicWall", new GalaxyPacketProcess.PacketHandler(this.HandleDynamicWall));
            GalaxyGameModule.GetGameManager<GalaxyPacketProcess>().AddPacketHandler("GPacketShieldWall", new GalaxyPacketProcess.PacketHandler(this.HandleShield));
        ...
        public void HandleDynamicWall(GPacketBase pkt)
        {
            sbyte flag = pkt.GetSByte("createFlag");
            int id = pkt.GetInt32("id");
            if (flag == 1)
            {
                float x = pkt.GetFloat("x");
                float y = pkt.GetFloat("y");
                float ex = pkt.GetFloat("ex");
                float ey = pkt.GetFloat("ey");
                float height = pkt.GetFloat("height");
                var srcAvatar = GalaxyGameModule.GetGameManager<GalaxyActorManager>().GetByServerID(pkt.mAvatarID) as ActorObj;
                float groundHeight = 0;
                if (null != srcAvatar)
                {
                    groundHeight = srcAvatar.GetPos().y;
                }
                int effectid = 800;//pkt.GetInt32("effectid");
                m_NavGrid.CreateDynamicWall(id, new Vector3(x, groundHeight, y), new Vector3(ex, groundHeight, ey), height, effectid);
            }
            else
            {
                m_NavGrid.DestroyDynamicWall(id);
            }
            
        }

        public void HandleShield(GPacketBase pkt)
        {
            sbyte flag = pkt.GetSByte("createFlag");
            int id = pkt.GetInt32("id");
            if (flag == 1)
            {
                float x = pkt.GetFloat("x");
                float y = pkt.GetFloat("z");
                float z = pkt.GetFloat("y");
                float dx = pkt.GetFloat("dx");
                float dy = pkt.GetFloat("dz");
                float dz = pkt.GetFloat("dy");
                float l = pkt.GetFloat("l");
                float w = pkt.GetFloat("w");
                float h = pkt.GetFloat("h");
                OBB obb = new OBB();
                obb.Center = new Vector3(x, y, z);
                obb.HalfExtent = new Vector3(w, h, l);
                obb.Orientation = Quaternion.FromToRotation(Vector3.forward, new Vector3(dx, dy, dz));
                m_NavGrid.AddShield(id, obb);
                
                //GameObject cub = GameObject.CreatePrimitive(PrimitiveType.Cube);
                //cub.transform.localPosition = new Vector3(x,y,z);
               // cub.transform.forward = new Vector3(dx, dy, dz);
               // cub.transform.localScale = new Vector3(w * 2, h * 2, l * 2);
                //TestObj.Add(id, cub);
                
            }
            else
            {
                m_NavGrid.RemoveShield(id);
                /*
                GameObject cub = null;
                if (TestObj.TryGetValue(id, out cub))
                {
                    if (cub != null)
                    {
                        GameObject.DestroyImmediate(cub);
                    }
                }
                */
            }
